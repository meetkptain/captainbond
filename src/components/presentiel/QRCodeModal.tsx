...
export function QRCodeModal({ roomCode, open, onClose }: QRCodeModalProps) {
  ...
  const inviteUrl = `${typeof window !== 'undefined' ? window.location.origin : 'https://captainbond.com'}/join/${roomCode}`;
  const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(`Jouons à Captain Bond ! 🔥 Rejoins-moi avec le code : ${roomCode}. ${inviteUrl}`)}`;

  const handleWhatsApp = () => {
    capture(AnalyticsEvents.SHARE_WHATSAPP, { room_code: roomCode });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
      <div className="bg-slate-900 border border-slate-800 p-8 rounded-3xl max-w-sm w-full text-center relative space-y-6 shadow-2xl">
        ...
        <button
          ...
          className="w-full py-3 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl transition-all cursor-pointer border-none flex items-center justify-center gap-2 text-sm"
        >
